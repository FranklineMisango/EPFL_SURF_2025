        ...
        try:
            ...
            for node in result.nodes:
                feature = OSMFeature(
                    ...
                    lon=float(node.lon),
                    tags=dict(node.tags),
                    distance_to_station=geodesic((lat, lon), (node.lat, node.lon)).meters
                )
            ...
            for way in result.ways:
                if hasattr(way, 'center_lat') and hasattr(way, 'center_lon'):
                    feature = OSMFeature(
                        ...
                        tags=dict(way.tags),
                        distance_to_station=geodesic((lat, lon), (way.center_lat, way.center_lon)).meters
                    )
                    features.append(feature)
            ...
            for relation in result.relations:
                if hasattr(relation, 'center_lat') and hasattr(relation, 'center_lon'):
                    feature = OSMFeature(
                        ...
                        distance_to_station=geodesic((lat, lon), (relation.center_lat, relation.center_lon)).meters
                    )
                    features.append(feature)
